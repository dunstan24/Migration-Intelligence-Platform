...
with open(r"d:\Interlace_DataAnalyst\Migration-Intelligence-Platform\tmp\final_bench_results.txt", "w", encoding="utf-8") as f:
    f.write(f"{'Case':<30} | {'Prob':<6} | {'Boost':<6} | {'Final':<6}\n")
    f.write("-" * 60 + "\n")
    ...
    for p, en, x, l in cases:
        ...
        f.write(f"{l:<30} | {p190:<6.3f} | {boost:<6.2f} | {min(score, 1.0):<6.3f}\n")
